...
local function do_discover(discovery_file, update)
  ...
  if update and vim.fn.filereadable(discovery_file) == 1 then
    ...
    table.insert(input, "")
    table.insert(input, "Output a discovery note with TWO sections:")
    table.insert(input, "")
    table.insert(input, "## Project Map")
    table.insert(input, "Organize by domain/subsystem. Use ### headings per domain.")
    table.insert(input, "Under each domain, list every relevant file as `path/to/file.ext` — short description.")
    table.insert(input, "Cover ALL files visible in the tree. The paths must work with vim gf navigation.")
    table.insert(input, "")
    table.insert(input, "## Candidate Files")
    table.insert(input, "List the 5-15 most important files to read first when getting oriented.")
    table.insert(input, "Format: `path/to/file.ext` — why this file matters.")
  end
